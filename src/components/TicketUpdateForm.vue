    ...
    updateTicket() {
      ...
      if (!this.updateFields.coordinatesX) {
        this.strTicket.coordinates.x < this.MIN_INTEGER ? this.errors.coordinatesX = `Значение coordinates.x не может быть меньше возможного ${this.MIN_INTEGER}` :
            this.strTicket.coordinates.x > this.MAX_INTEGER ? this.errors.coordinatesX = `Значение coordinates.x не может быть больше возможного ${this.MAX_INTEGER}` : ticket.coordinates.x = this.strTicket.coordinates.x
      }
      if (!this.updateFields.coordinatesY) {
        this.strTicket.coordinates.y < this.MIN_DOUBLE ? this.errors.coordinatesY = `Значение coordinates.y не может быть меньше возможного ${this.MIN_DOUBLE}` :
            this.strTicket.coordinates.y > this.MAX_DOUBLE ? this.errors.coordinatesY = `Значение coordinates.y не может быть больше возможного ${this.MAX_DOUBLE}` : ticket.coordinates.y = this.strTicket.coordinates.y
      }
      if (!this.updateFields.price) {
        this.strTicket.price < 0 ? this.errors.price = `Значение price не может быть меньше 0` :
            this.strTicket.price > this.MAX_INTEGER ? this.errors.price = `Значение price не может быть больше возможного ${this.MAX_INTEGER}` : ticket.price = this.strTicket.price
      }
      if (!this.updateFields.discount) {
        this.strTicket.discount < 0 ? this.errors.discount = `Значение discount не может быть меньше 0` :
            this.strTicket.discount > 100 ? this.errors.discount = `Значение discount не может быть больше 100` : ticket.price = this.strTicket.discount
      }
      ...
      if (this.hasPerson) {
        if (!this.updateFields.personHeight) {
          this.strTicket.person.height <= 0 ? this.errors.height = `Значение person.height не может быть меньше 1` :
              this.strTicket.person.height > 300 ? this.errors.height = `Значение person.height не может быть больше 300` : ticket.person.height = this.strTicket.person.height
        }
        if (!this.updateFields.locationX) {
          this.strTicket.person.location.x < this.MIN_INTEGER ? this.errors.locationX = `Значение location.x не может быть меньше возможного ${this.MIN_INTEGER}` :
              this.strTicket.person.location.x > this.MAX_INTEGER ? this.errors.locationX = `Значение location.x не может быть больше возможного ${this.MAX_INTEGER}` : ticket.person.location.x = this.strTicket.person.location.x
        }
        if (!this.updateFields.locationY) {
          this.strTicket.person.location.y < this.MIN_LONG ? this.errors.locationY = `Значение location.y не может быть меньше возможного ${this.MIN_LONG}` :
              this.strTicket.person.location.y > this.MAX_LONG ? this.errors.locationY = `Значение location.y не может быть больше возможного ${this.MAX_LONG}` : ticket.person.location.y = this.strTicket.person.location.y
        }
        if (!this.updateFields.locationZ) {
          this.strTicket.person.location.z < this.MIN_DOUBLE ? this.errors.locationZ = `Значение location.z не может быть меньше возможного ${this.MIN_DOUBLE}` :
              this.strTicket.person.location.z > this.MAX_DOUBLE ? this.errors.locationZ = `Значение location.z не может быть больше возможного ${this.MAX_DOUBLE}` : ticket.person.location.z = this.strTicket.person.location.z
        }
